...
function InteractionWall({ posts, hotTags, starredPosts, view, onViewChange, onClose, onToggleStar, onOpen, onEdit, onDelete, canManage }: { posts: Post[]; hotTags: string[]; starredPosts: string[]; view: "all" | "starred"; onViewChange: (view: "all" | "starred") => void; onClose: () => void; onToggleStar: (id: string) => void; onOpen: (post: Post) => void; onEdit: (post: Post) => void; onDelete: (post: Post) => void; canManage: (post: Post) => boolean }) {
  ...
  const openAfterSingleClick = (post: Post) => { clickTimer.current = setTimeout(() => onOpen(post), 220); };
  const starAfterDoubleClick = (post: Post) => { if (clickTimer.current) clearTimeout(clickTimer.current); onToggleStar(post.id); };
  return <div className="wall-overlay" onMouseDown={() => { setContextPost(null); }}><section className="interaction-wall" onMouseDown={(event) => event.stopPropagation()}><header className="wall-head"><div><span className="eyebrow">LIVE IDEA SPACE</span><h2>互動牆 <small>{shown.length} 張想法</small></h2></div><div className="wall-actions"><button className={view === "all" ? "wall-tab active" : "wall-tab"} onClick={() => onViewChange("all")}>全部留言</button><button className={view === "starred" ? "wall-tab active" : "wall-tab"} onClick={() => onViewChange("starred")}><Star size={15} fill={view === "starred" ? "currentColor" : "none"}/> 已保留 {starredPosts.length}</button><button className="wall-close" onClick={onClose}><X size={21}/></button></div></header><div className="wall-guide"><span>雙擊便利貼即可{view === "all" ? "保留" : "取消保留"} <Star size={14}/></span><span>點一下開啟討論</span><span>自己的留言可按右鍵編輯或刪除</span></div>{hotTags.length > 0 && <div className="wall-tags"><button className={!tag ? "active" : ""} onClick={() => setTag("")}>所有標籤</button>{hotTags.map((item) => <button className={tag === item ? "active" : ""} key={item} onClick={() => setTag(item)}>#{item}</button>)}</div>}<div className="wall-canvas">{shown.map((post, index) => <article className={`wall-note note-${index % 6} ${starredPosts.includes(post.id) ? "starred" : ""}`} key={post.id} onDoubleClick={() => starAfterDoubleClick(post)} onClick={() => openAfterSingleClick(post)} onContextMenu={(event) => { event.preventDefault(); if (canManage(post)) setContextPost(post); }}><div className="note-top"><span className={`pill ${categoryStyle[post.category]}`}>{post.category}</span>{starredPosts.includes(post.id) && <Star className="note-star" size={15} fill="currentColor"/>}</div><h3>{post.title}</h3><p>{post.content}</p><footer><span>{post.author}</span><span>{post.tags.slice(0, 2).map((item) => `#${item}`).join(" ")}</span></footer></article>)}{shown.length === 0 && <div className="wall-empty"><Star size={30}/><b>{view === "starred" ? "還沒有保留的想法" : "沒有符合的想法"}</b><p>雙擊任一張留言，就會留在你的星號區。</p></div>}</div>{contextPost && <div className="wall-context"><b>{contextPost.title}</b><button onClick={() => onEdit(contextPost)}><Pencil size={14}/> 編輯</button><button className="danger" onClick={() => { void onDelete(contextPost); }}><Trash2 size={14}/> 刪除</button></div>}</section></div>;
}
